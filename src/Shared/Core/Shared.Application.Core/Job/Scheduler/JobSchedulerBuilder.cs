...
/// <list type="bullet">
/// <item><description><see cref="AddJob{TJob}(JobSchedule)"/> — для задач, реализующих <see cref="IScheduledJob"/>.</description></item>
/// <item><description><see cref="AddJob(string, JobSchedule, Func{IServiceProvider, CancellationToken, Task})"/> — для задач, заданных делегатом.</description></item>
/// <item><description><see cref="AddCron(string, string, Func{IServiceProvider, CancellationToken, Task})"/>, <see cref="AddFlags(string, JobTriggerFlags, TimeSpan, Func{IServiceProvider, CancellationToken, Task})"/> — упрощённая запись.</description></item>
/// </list>
/// </para>
/// </summary>
public sealed class JobSchedulerBuilder
{
        ...
        where TJob : class, IScheduledJob
    {
        ...
        return AddJob(typeof(TJob), schedule);
    }
        ...
        Func<IServiceProvider, CancellationToken, Task> action)
    {
        ...
        _definitions.Add(new JobDefinition(jobKey, action, schedule));
        return this;
    }
        ...
        Func<IServiceProvider, CancellationToken, Task> action)
    {
        ...
        return AddJob(jobKey, new JobSchedule.Cron(cronExpression), action);
    }
        ...
        Func<IServiceProvider, CancellationToken, Task> action)
    {
        ...
        return AddJob(jobKey, new JobSchedule.Flags(flags, specificTime), action);
    }
