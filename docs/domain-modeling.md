...
Domain-слой — это **ядро** Clean Architecture. Он содержит бизнес-сущности, value objects и бизнес-исключения. Не зависит от внешних библиотек, фреймворков или infrastructure-концернов. Lifecycle-действия — в Application-слое (см. [Lifecycle Actions](lifecycle-actions.md)).
...
| **Lifecycle Actions** | Side-эффекты (аудит, нотификации, обогащение) — отдельные `ILifecycleActionHandler<TEntity>` в Application-слое |
...
├── Cache/                   ← MemoryCacheExtensions, ключи кэша
...
└── LifecycleAction/         ← Реализация lifecycle-действий
    ├── Interfaces/          ← ILifecycleActionHandler, ILifecycleActionOrchestrator, ILifecycleActionGate, ILifecycleEntityRegistry
    ├── Extensions/          ← DependencyInjectionExtensions (AddLifecycleActions)
    ├── EntityKey.cs
    ├── LifecycleActionHandlerBase.cs
    ├── LifecycleActionOrchestrator.cs
    ├── LifecycleActionGate.cs
    ├── LifecycleEntityRegistry.cs
    └── LifecycleActionValidator.cs
```

> **Примечание:** lifecycle-действия живут в Application-слое (`Shared.Application.Core/LifecycleAction/`). Enum `LifecyclePhase` — в `Shared.Domain.Core/Enums/`. Сущности не реализуют lifecycle-логику — обработчики регистрируются отдельно как `ILifecycleActionHandler<TEntity>`. См. [Lifecycle Actions](lifecycle-actions.md).
...
> **Изменение:** `EntityBase<TKey>` реализует только `IEntity<TKey>` — минимум, без side-effect-логики. Lifecycle-функционал вынесен в `ILifecycleActionHandler<TEntity>` (см. [Lifecycle Actions](lifecycle-actions.md)).
    ...
    where TEntity : class, IEntity<TKey>
{
    ...
    // Сеттеры audit-полей (вызываются из OnCreate/OnUpdate/OnDelete)
    public void SetCreatedByUserId(Guid? userId);
    public void SetCreatedByUserName(string? userName);
    public void SetDateCreated(DateTime dateCreated);
    public void SetUpdatedByUserId(Guid? userId);
    public void SetDateUpdated(DateTime? dateUpdated);
    public void SetDeletedByUserId(Guid? userId);
...
> См. [Lifecycle Actions](lifecycle-actions.md) — полный разбор архитектуры, `ILifecycleActionHandler`, `ILifecycleActionOrchestrator`, `ILifecycleActionGate`, `ILifecycleEntityRegistry`.
