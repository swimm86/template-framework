...
Repository Pattern в фреймворке Shared предоставляет централизованный доступ к данным через три интерфейса, разделённых по принципу **Interface Segregation Principle** (ISP):

- **`IGetterRepository<TEntity>`** — read-only контракт: запросы, проекции, агрегации, группировки.
- **`ISetterRepository<TEntity>`** — write-only контракт: добавление, обновление, удаление, `Execute`, `SaveChanges`.
- **`IRepository<TEntity>`** — составной контракт: наследует оба интерфейса и добавляет `Set(QueryOptions<TEntity>?)`.

`IRepository<T>` — это **рабочий контракт по умолчанию**: именно его возвращает `IUnitOfWork.GetRepository<T>()`. Узкие `IGetterRepository<T>` и `ISetterRepository<T>` — это **опциональные ISP-альтернативы** для кода, в котором действительно нужна только одна сторона (проекционный сервис, pure-write пакетный обработчик). В DI все три интерфейса резолвятся в один и тот же `EfRepository<T>` поверх общего `DbContext` (см. [DI registration](#di-registration)).
...
### Организация контракта (partial-файлы)

`IGetterRepository<TEntity>` и `ISetterRepository<TEntity>` разделены на `partial`-файлы по группам методов:

| Интерфейс | Partial-файл | Содержимое |
|-----------|--------------|------------|
| `IGetterRepository<TEntity>` | `IGetterRepository.Get.cs` | `GetAsync`, `GetRangeAsync`, `FirstOrDefaultAsync`, `SingleOrDefaultAsync`, `LastOrDefaultAsync`, `GetGroupingAsync` |
| `IGetterRepository<TEntity>` | `IGetterRepository.Aggregation.cs` | `CountAsync`, `AnyAsync`, `SumAsync`, `CountGroupsAsync`, `MaxAsync`, `MinAsync` |
| `ISetterRepository<TEntity>` | `ISetterRepository.cs` | `Execute`, `ExecuteAsync`, `SaveChanges` |
| `ISetterRepository<TEntity>` | `ISetterRepository.Add.cs` | `AddAsync`, `AddRangeAsync` |
| `ISetterRepository<TEntity>` | `ISetterRepository.Update.cs` | `ExecuteUpdateRangeAsync` + `GetUpdateRangeAsyncLambdaFunc` |
| `ISetterRepository<TEntity>` | `ISetterRepository.Remove.cs` | `RemoveAsync`, `RemoveRangeAsync`, `RemovePermanentRangeAsync`, `ExecuteRemoveRangeAsync` |
| `IRepository<TEntity>` | `IRepository.cs` | `Set(QueryOptions<TEntity>?)` + объединение `IGetterRepository` и `ISetterRepository` |

Исходники: `src/Shared/Core/Shared.Domain.Core/Dal/Repository/Interfaces/`.

## Когда использовать какой интерфейс

Контракт по умолчанию — **`IRepository<T>`**. Узкие интерфейсы — это ISP-инструмент для случаев, когда код **доказуемо** использует только одну сторону.

| Интерфейс | Когда уместен |
|-----------|---------------|
| `IRepository<T>` | **По умолчанию.** Все встроенные command/query-handler'ы (`CreateCommandHandler`, `UpdateCommandHandler`, `DeleteCommandHandler`, `CloneCommandHandler`, `ReadQueryHandler`, `ReadListQueryHandler`) наследуют `EntityRequestHandler.Repository : IRepository<T>` и используют его. Стандартный CRUD — это почти всегда «прочитать, потом изменить», что требует обоих контрактов. |
| `IGetterRepository<T>` | Опционально: read-only код, которому действительно не нужны методы записи (аналитические query-handler'ы, проекции, тестовые fake'и). |
| `ISetterRepository<T>` | Опционально: pure-write код, который только вызывает `ExecuteUpdateRangeAsync` / `ExecuteRemoveRangeAsync` / `AddAsync` / `RemoveAsync` и не читает сущность перед записью (фоновые миграции, пакетные обработчики, тестовые fake'и). **Не подходит для стандартных CRUD-handler'ов** — они сначала читают сущность через `GetByIdOrThrowAsync` / `GetAsync`. |

## Интерфейсы

Все три интерфейса определены в `Shared.Domain.Core.Dal.Repository.Interfaces`, где `TEntity : class, IEntity`. Полный XML-doc — в исходниках (см. таблицу выше). Ниже — обзор методов по группам; подробные примеры и поведение — в следующих разделах.

### IGetterRepository<TEntity>

Read-only контракт. Состоит из двух partial-частей: методов запросов (`Get.cs`) и агрегаций (`Aggregation.cs`).

#### Чтение (`IGetterRepository.Get.cs`)
...
#### Агрегация (`IGetterRepository.Aggregation.cs`)

`CountAsync`, `AnyAsync`, `SumAsync`, `CountGroupsAsync<TKey>`, `MaxAsync<TOut>`, `MinAsync<TOut>` — каждая с перегрузками по `QueryOptions` / `ISpecification`.

### ISetterRepository<TEntity>

Write-only контракт. Состоит из четырёх partial-частей: `Execute`/`SaveChanges` (основной файл), `Add.cs`, `Update.cs`, `Remove.cs`.

#### Добавление (`ISetterRepository.Add.cs`)

| Метод | Назначение |
|-------|------------|
| `AddAsync` / `AddRangeAsync` | Добавление (принимает `userId`, `userName` для аудита) |

#### Обновление (`ISetterRepository.Update.cs`)

| Метод | Назначение |
|-------|------------|
| `ExecuteUpdateRangeAsync(predicate / options / specification, ...)` | DB-level пакетное обновление через `ExecuteUpdateAsync` |

#### Удаление (`ISetterRepository.Remove.cs`)
...
#### Выполнение и сохранение (`ISetterRepository.cs`)
...
Составной контракт. Объединяет все методы `IGetterRepository<TEntity>` и `ISetterRepository<TEntity>` плюс собственный:

- `Set(QueryOptions<TEntity>?)` — доступ к `IQueryable<TEntity>` с применёнными настройками.

`Set` намеренно вынесен на `IRepository<T>`, а не на узкие интерфейсы: прямой `IQueryable` обычно нужен в коде, который комбинирует чтение и запись (UnitOfWork-обёртки, фоновые задачи). Для read-only кода используйте методы `IGetterRepository<T>`.

### Вспомогательный статический метод

Статический метод `GetUpdateRangeAsyncLambdaFunc<TProp>(propertyExpression, valueExpression)` определён в `ISetterRepository.Update.cs` и формирует кортеж `(LambdaExpression, LambdaExpression)` для `ExecuteUpdateRangeAsync`. Доступен через тип любого из трёх интерфейсов (`IRepository<TEntity>`, `IGetterRepository<TEntity>`, `ISetterRepository<TEntity>`).
...
Вызывается **через тип** (`IRepository<TEntity>` / `IGetterRepository<TEntity>` / `ISetterRepository<TEntity>`), а не через экземпляр:
...
`EfRepository<T>` регистрируется в `AddDbContext<TSettings, TContext>` (`src/Shared/Dal/Shared.Infrastructure.Dal.EFCore/Extensions/ServiceCollectionExtensions.cs`) под всеми тремя интерфейсами как **Scoped**-сервис:

```csharp
// Все три интерфейса резолвятся в один scoped EfRepository<T> поверх общего DbContext.
// Это нужно, чтобы в рамках одного scope инжекция любого из трёх интерфейсов
// давала один экземпляр с тем же ChangeTracker — иначе reads и writes
...
**Зачем три регистрации, а не одна:**

- `IRepository<>` — используется `IUnitOfWork.GetRepository<T>()` и `EntityRequestHandler.Repository`. Это **рабочий контракт по умолчанию**.
- `IGetterRepository<>` — опциональная ISP-альтернатива для read-only потребителей (см. [Когда использовать какой интерфейс](#когда-использовать-какой-интерфейс)).
- `ISetterRepository<>` — опциональная ISP-альтернатива для pure-write потребителей.

Все три резолвятся в **один и тот же scoped-экземпляр**, потому что в рамках HTTP-запроса должен быть ровно один `DbContext` (и один `ChangeTracker`): если бы, например, `IGetterRepository<T>` и `IRepository<T>` жили в разных скоупах, read в Getter не увидел бы pending changes из Setter, и транзакция потеряла бы смысл.

> Не удаляйте ни одну из трёх регистраций — это намеренное shared-scope-связывание, и в исходнике `ServiceCollectionExtensions.cs` есть соответствующий комментарий.
...
9. **По умолчанию используйте `IRepository<T>`**
   - Встроенные command/query-handler'ы и `IUnitOfWork.GetRepository<T>()` возвращают именно его. Узкие `IGetterRepository<T>` / `ISetterRepository<T>` оправданы только в коде, который **доказуемо** использует одну сторону (аналитические проекции, фоновая pure-write миграция). См. [«Когда использовать какой интерфейс»](#когда-использовать-какой-интерфейс).
