...
- **UsePresentationCore()** — one-call настройка всего presentation middleware
...
Значение `template` подставляется из `Constants.AppName = "Template"` (`./src/Services/Common/Template.Presentation/Constants.cs`). Сегмент `getter` задаётся атрибутом `[ControllerType("getter")]` на `GetterControllerBase`. Имя контроллера (`persons`) формируется `ControllerNameConvention` из имени класса `PersonsController`.
...
**Пример:** реальный `PersonsController` из `./src/Services/Getter/Template.Getter.Api/Controllers/PersonsController.cs`:
...
Имя контроллера `persons` (без суффикса `Controller`) и сегмент `getter` подставляются конвенциями из имени класса и `[ControllerType("getter")]` соответственно.
...
## HttpRequestExtensions — Correlation ID

Расширения для работы с Correlation ID находятся в `Shared.Application.Core.CorrelationId.Extensions.HttpRequestExtensions` (`./src/Shared/Core/Shared.Application.Core/CorrelationId/Extensions/HttpRequestExtensions.cs`).

```csharp
using Shared.Application.Core.CorrelationId.Extensions;

// Получение Correlation ID из заголовка
var correlationId = request.GetCorrelationId(); // Guid?

// Добавление Correlation ID (если отсутствует валидное значение)
request.TryAddCorrelationId();
```

> `CorrelationIdMiddleware` (`Shared.Presentation.Core.CorrelationId.Middlewares`) уже вызывает `TryAddCorrelationId()` для каждого входящего запроса внутри `UsePresentationCore()`, поэтому в контроллерах ручной вызов обычно не требуется.

### Cookie-расширения HttpRequest

`HttpRequestExtensions` в `Shared.Presentation.Core` (`./src/Shared/Core/Shared.Presentation.Core/Extensions/HttpRequestExtensions.cs`) предоставляет единственный публичный помощник для работы с cookies:

| Метод | Описание |
|-------|----------|
| `ContainsCookie(this HttpRequest, string cookieName)` | Проверяет наличие cookie по имени. Возвращает `true`, если cookie присутствует в запросе |

`TryGetCookieValue` объявлен как `private static` и не является частью публичного API.
